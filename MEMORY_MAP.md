...
| `0x403FFA`, `0x403FFB` | **CONFIRMED: "Front Panel Buffer U9301"** / **"Front Panel Buffer U9302"** (Table 3-1) | Confirms the long-standing "front-panel key/encoder status is the leading candidate" guess for `scheduler_tick_service`'s `[0x758]`/`[0x759]` source exactly - these are the two octal bus-driver ICs (U9301/U9302) that pass Storage Panel Control switch positions to the microprocessor (per the manual's "Store Panel Controls and Buffer" theory-of-operation section) |
