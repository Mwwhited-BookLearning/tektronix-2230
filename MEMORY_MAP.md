...
| `0x41000`, `0x42000` | Single-byte read ports (candidate, purpose unconfirmed) | `SUB_E4429`/`SUB_E440A` each read exactly one fixed-address byte and return it; not yet renamed pending identification (front-panel status? CRT controller status?) |
