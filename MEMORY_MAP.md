...
| `0x41000`, `0x42000` | Single-byte read ports, `read_channel1_status`/`read_channel2_status` (candidate - likely per-channel front-end status, not confirmed) | Called alternately in a self-test loop that takes repeated readings, spaced by `wait_readout_tick` delays - consistent with a per-channel status/calibration readback on a 2-channel scope, but not confirmed against a schematic |
